...
import os
from core.utils_export import generer_zip_archives, generer_zip_complet
...
def communication_eleve(request):
    ...
    if classe:
        ...
        professeur = ProfilUtilisateur.objects.filter(
            ...
            """Wrapper d'export : délègue la génération à `core.utils_export`.

            Utilise `generer_zip_archives(annee, categorie)` qui renvoie ``(zip_bytes, nb, errs)``.
            """
            ...
            zip_bytes, nb, errs = generer_zip_archives(annee, categorie)
            ...
            if errs:
                messages.warning(request, f'⚠️ Génération incomplète ({len(errs)} erreurs).')
        ...
        def export_annuel_complet(request):
            """
            Télécharge un ZIP complet de fin d'année :
            évaluations, travaux rendus, archives, QCM, cours.
            """
            ...
            if nb == 0:
                messages.warning(request, '⚠️ Aucun document à exporter pour cette année.')
                return redirect('core:archives')

            response = HttpResponse(zip_bytes, content_type='application/zip')
            response['Content-Disposition'] = f'attachment; filename="export_complet_{annee}.zip"'
            return response
        return None
...
# ─── Sous-fonctions privées de statistiques ───────────────────────────────────
...
# ─── Sous-fonctions privées de creer_evaluation ───────────────────────────────
...
# ✅ Alias — les deux noms pointent vers la même vue
...
# =====================================================
# ÉLÈVE — FICHE COMPLÈTE (contrat + évaluation)
# =====================================================

# ─── Sous-fonctions privées de eleve_fiche_complete ───────────────────────────
...
# =====================================================
# MODES OPÉRATOIRES — VUES PROFESSEUR
# =====================================================
